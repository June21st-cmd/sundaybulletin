...
- **커밋 메시지 권고 사항 (Commit Message Convention)**:
  - **한국어 작성 원칙**: 커밋 메시지의 설명 내용은 **한국어로 작성**하는 것을 권고합니다.
  - **접두어 및 이슈 번호 유지**: 영문 접두어(`<type>:`)와 이슈 번호(`#<이슈번호>`)는 기존 형식을 그대로 유지합니다.
  - **권고 형식**: `<타입>: #<이슈번호> <한국어 설명>` 또는 `<타입>: <한국어 설명>`
    - 예시: `feat: #12 한글 HWPX 템플릿 치환 엔진 추가`, `fix: 성경 본문 줄바꿈 오류 수정`, `docs: 윈도우 설치 가이드 보완`
  - **유연성 유지**: 강하게 차단하거나 통제하기보다 자연스러운 작업 흐름을 위한 권고 사항으로 적용합니다.
